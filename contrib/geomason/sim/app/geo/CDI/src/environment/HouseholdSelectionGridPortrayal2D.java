...
public class HouseholdSelectionGridPortrayal2D extends FieldPortrayal2D
    {
    public DrawPolicy policy;
    public GUIState state;
    public HouseholdSelectionGridPortrayal2D()
        {
        super();
        }

    /** @deprecated Use setDrawPolicy. */
    public HouseholdSelectionGridPortrayal2D (DrawPolicy policy)
        {
        super();
        this.policy = policy;
        }
    ...
    protected void hitOrDraw(Graphics2D graphics, DrawInfo2D info, Bag putInHere)
        ...
        if (policy != null && graphics != null)
            ...
            for(int x=0;x<objects.numObjs;x++)
                ...
                if (loc.x >= startx -2 && loc.x < endx + 4 &&
                    ...
                    if (graphics == null)
                        ...
                        if (portrayal.hitObject(portrayedObject, newinfo))
                            {
                            tempBag.add(getWrapper(portrayedObject));
                            }
                        }
                    else
                        {
            	...
            	if(putInHere!=null && tempBag.numObjs>0)
            	{
            		NorthLandsMovementWithUI ui = (NorthLandsMovementWithUI)info.gui;
            		int index = ui.guirandom.nextInt(tempBag.numObjs);
            		putInHere.add(tempBag.get(index));
            		
            		//System.out.println(putInHere.numObjs);
            	}
    ...
    public LocationWrapper getWrapper(Object object)
        ...
        final SparseGrid2D field = (SparseGrid2D) this.field;
        //field
        final StableInt2D w = new StableInt2D(this, object,state);
